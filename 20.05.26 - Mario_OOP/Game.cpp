...
void Game::horizon_move_map(const float dx) {
	...
	const float testX = mario->get_x() - dx;
...
MovingObject* Game::create_moving(
	const float x,
	const float y,
	const float w,
	const float h,
	...
	MovingObject* item = static_cast<MovingObject*>(moving.back().get());
	item->set_horiz_speed(0.2f);
	++movingLength;
	return item;
}
...
void Game::put_object_on_map(const BaseObject& obj) {
	map.put_object_on_map(obj);
}

void Game::put_object_on_map(const Mario& player) {
	map.put_object_on_map(player);
}

void Game::keyboard_detect(
	int& moveDirection,
	bool& jumpRequested,
	bool& shouldExit
) {
...
void Game::run() {
	...
	do {
		...
		if (moveDirection != MOVE_STOP) {
			horizon_move_map(
				static_cast<float>(moveDirection) * HORIZONTAL_SPEED
			);
		}
		...
		for (int i = 0; i < movingLength; ++i) {
			...
			if (moving[i]->get_y() > map.get_height()) {
				delete_moving(i);
				--i;
				continue;
			}
