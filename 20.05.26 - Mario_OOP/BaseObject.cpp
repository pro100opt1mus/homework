...
void MovingObject::vert_move(Game& game) {
	IsFly = true;
	vertSpeed += GRAVITY;
	...
	for (int i = 0; i < game.get_brick_length(); i++) {
		if (BaseObject::is_collision(*this, *game.get_bricks()[i])) {
			if (((*game.get_bricks()[i]).get_cType() == MYSTERY_BLOCK) &&
				(vertSpeed < 0)) {
				(*game.get_bricks()[i]).set_cType(EMPTY_BLOCK);
				MovingObject* item = game.create_moving(
					game.get_bricks()[i]->get_x(),
					game.get_bricks()[i]->get_y() - 3,
					3,
					2,
					COLLECTIBLE
				);

				item->set_vert_speed(ITEM_BOUNCE_SPEED);
			}
			...
			if (((*game.get_bricks()[i]).get_cType() == EXIT_BLOCK) &&
				(cType == PLAYER)) {
				game.rise_level();
...
void MovingObject::horiz_move(Game& game) {
	x += horizSpeed;
